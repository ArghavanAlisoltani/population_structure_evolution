...
python GWAS_TMRCA_join_and_plots_v12.py \
  ...
  --outdir "gwas_MTAG_trait_tmrca_results_v12_filtered" \
...
def summarize_snps_below_mean(annotated, trait_col, mean_col, global_mean):
    """
    Flag SNPs whose TMRCA < global_mean (computed from original TMRCA file).

    Returns:
      overall_df, by_trait_df, annotated_with_flag
    """
    col_flag = "below_global_mean_tmrca"

    tm = annotated[mean_col]
    flag = tm < global_mean
    flag = flag & tm.notna()     # ensure NA stay False
...
def scatter_by_trait(df, xcol, ycol, trait_col, ax, title,
    ...
    overridden by xlabel_override (e.g., 'TMRCA' or 'TMRCA (log10 scale)').
    """
    ...
    # ----- overall stats (across all traits)
    ...
    for i, t in enumerate(traits):
        ...
        # regression / stats for this trait
        mask = m_plot.copy()
        if reg_filter is not None:
            mask &= (x > reg_filter)
        x_reg = x[mask]
        ...
        # per-trait stats in legend (short form)
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            ...
            "(v12: adds log10(TMRCA) plotting & regression + all v11 features)."
        )
    )
    ...
    # Allow excluding traits from colored-by-trait figures entirely
    ...
    # Control per-trait stats text position (quantiles of x/y within trait)
    ...
    if args.tmrca_log10:
        # log10(TMRCA); rows with TMRCA <= 0 become NaN and are removed from plots/regression
