...
python GWAS_TMRCA_join_and_plots_v4.py \
  ...
  --outdir "gwas_MTAG_trait_tmrca_results_v4" \
...
python GWAS_TMRCA_join_and_plots_v4.py \
  ...
  --tmrca "/Users/aria/Desktop/OSU_projects/conifers/LP/ARGweaver/NOV_27_2025/all_tmrca_as_scaffold1.tsv" \
  --outdir "gwas_single_trait_tmrca_results_v4" \
  --regline --show-lm --show-r --show-p
'''
# Optional: Pearson r, p via SciPy
try:
...
def scatter_by_trait(df, xcol, ycol, trait_col, ax, title,
    ...
    # Global annotation across the filtered subset (if not per-trait stats)
...
def filter_by_min_points(df, trait_col, xcol, ycol, min_n):
    """Keep only traits with >= min_n finite x & y points."""
    if min_n <= 0:
...
def main():
    ap = argparse.ArgumentParser(description="Annotate GWAS with TMRCA & summarize/plot (v4; trait min-N filtering for colored plots)")
    ...
    # NEW: minimum points per trait for colored-by-trait plots
    ap.add_argument("--min-n-trait", type=int, default=0,
                    help="Exclude traits with < N points (only in colored-by-trait plots). Default: 0 (no filtering).")
    ...
    scatter_basic(dfp[c_beta].to_numpy(), dfp["mean_tmrca"].to_numpy(), ax,
                  ...
                  show_lm=args.show_lm, show_r=args.show_r, show_p=args.show_p, annot_loc=args.annot_loc)
