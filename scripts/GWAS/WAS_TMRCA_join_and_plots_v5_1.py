...
python GWAS_TMRCA_join_and_plots_v5.1.py \
...
def merge_asof_per_chrom(gwas, tmrca, scaf_col, pos_col,
    ...
    """
    For each scaffold:
      - asof-merge SNPs to nearest segment start at/left of SNP pos
      - if pos ∈ [start, end] -> keep TMRCA; else NULL *all* TMRCA fields
    ...
    for scaf, gsub in gwas.groupby(scaf_col, sort=False):
        ...
        # Clear ALL TMRCA columns for non-inside matches
...
def trait_summary(df, trait_col, tmrca_col):
    """
    Summaries use NaN-aware stats and therefore *exclude NA TMRCA* from
    median/min/max/quantiles. We also report n_total and n_with_tmrca.
    """
    def one(g):
        x = g[tmrca_col]
        has = x.notna().any()
...
def pearson_r_p(x, y):
    ...
    # Fallback: r only (p requires SciPy)
...
def place_outside_text(fig, ax, text, position="top", color="black", margin=0.18):
    """
    Place annotation text outside the axes with adjustable margin.
    margin = fraction of the figure reserved for the text block.
    """
    if not text:
        return
    margin = float(np.clip(margin, 0.05, 0.35))
...
def scatter_basic(x, y, ax, title, xlabel, ylabel,
    ...
    if show_lm or show_r or show_p:
        ...
        if stats_outside:
            place_outside_text(ax.figure, ax, txt, position=stats_position,
                               color=color, margin=stats_margin)
        else:
...
def scatter_by_trait(df, xcol, ycol, trait_col, ax, title,
                     regline=False, show_lm=False, show_r=False, show_p=False,
                     per_trait_stats=False, stats_outside=False, stats_position="top", stats_margin=0.18):
    traits = df[trait_col].astype(str).fillna("NA").unique().tolist()
    cmap = cm.get_cmap("tab20", max(10, len(traits)))

    # Overall stats across the filtered subset
    if (show_lm or show_r or show_p) and not per_trait_stats and not df.empty:
        ...
        if stats_outside:
            place_outside_text(ax.figure, ax, txt, position=stats_position,
                               color="black", margin=stats_margin)
        else:
    ...
    for i, t in enumerate(traits):
        ...
        # Per-trait stats in legend (keeps figure clean)
...
def main():
    ap = argparse.ArgumentParser(
        description="Annotate GWAS with TMRCA & summarize/plot (v5.1: NA-safe, outside stats margin, color for all-points, min-N per trait)"
    )
    ...
    ap.add_argument("--stats-margin", type=float, default=0.18,
                    help="Figure fraction reserved for outside stats text (e.g., 0.25 if overlapping).")

    # Color for all-points (non-trait) plots
    ap.add_argument("--allpoints-color", default="#000000",
                    help="Color for all-points figures (single color).")

    # Minimum points per trait (colored-by-trait plots only)
    ap.add_argument("--min-n-trait", type=int, default=0,
                    help="Exclude traits with < N finite points in colored plots")
    ...
    # interval join (keep only TMRCA columns we need)
    ...
    # trait summary (stats exclude NA TMRCA automatically)
    ...
    # Exclude NA TMRCA from all downstream calcs/figures
    ...
    scatter_basic(dfp[c_beta].to_numpy(), dfp["mean_tmrca"].to_numpy(), ax,
                  ...
                  stats_outside=args.stats_outside,
                  stats_position=args.stats_position, stats_margin=args.stats_margin)
    ...
    # MAF vs mean TMRCA (colored by trait)
