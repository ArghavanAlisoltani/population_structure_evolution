...
def scatter_by_trait(df, xcol, ycol, trait_col, ax, title,
    ...
    for i, t in enumerate(traits):
        ...
        if (str(t) in stats_traits) and (show_lm or show_r or show_p) and x_reg.size >= 2:
            txt = format_stats_text(x_reg, y_reg, show_lm, show_r, show_p)
            if txt:
                # place near top-right of that trait’s point cloud
                try:
                    x_pos = np.nanpercentile(x, 80)
                    y_pos = np.nanpercentile(y, 95)
                except Exception:
                    x_pos = np.nanmean(x)
                    y_pos = np.nanmax(y)
...
def main():
    ap = argparse.ArgumentParser(
        description="Annotate GWAS with TMRCA & summarize/plot (v9: trait-specific alpha, colors, TMRCA regfilter)."
    )
    ...
    # NEW: regression-only filter (all points shown, but lines/stats only above this TMRCA)
