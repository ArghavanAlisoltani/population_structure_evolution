...
def merge_asof_per_chrom(gwas, tmrca, scaf_col, pos_col,
    ...
    for scaf, gsub in gwas.groupby(scaf_col, sort=False):
        ...
        # --- CRITICAL FIX: make join keys the same dtype (float64) and sorted
...
def q4(series): return np.nanpercentile(series, 100.0)
...
def add_reg_line(ax, x, y, color="black", lw=1.2, alpha=0.8):
    ...
    xs = np.linspace(np.nanmin(x[msk]), np.nanmax(x[msk]), 200)
...
def scatter_basic(x, y, ax, title, xlabel, ylabel,
    ...
    if regline: add_reg_line(ax, x, y, color=color)
    ax.set_title(title); ax.set_xlabel(xlabel); ax.set_ylabel(ylabel); ax.grid(alpha=0.15)
...
def scatter_by_trait(df, xcol, ycol, trait_col, ax, title, regline=False):
    ...
    for i, t in enumerate(traits):
        ...
        ax.scatter(sub[xcol], sub[ycol], s=10, alpha=0.7, color=cmap(i), label=t, edgecolors="none")
        if regline: add_reg_line(ax, sub[xcol].to_numpy(), sub[ycol].to_numpy(), color=cmap(i))
    ax.set_title(title); ax.set_xlabel(xcol); ax.set_ylabel(ycol)
    ax.legend(markerscale=1.5, fontsize=8, ncol=2, frameon=False); ax.grid(alpha=0.15)
...
def main():
    ...
    gcols, tcols = gwas.columns.tolist(), tmrca.columns.tolist()
    ...
    # numeric coercions (note: keep as float to tolerate NaN)
    ...
    scatter_by_trait(dfp, xcol=c_beta, ycol="mean_tmrca",
                     ...
                     title="BETA vs mean TMRCA (by trait)", regline=args.regline)
    ...
    scatter_by_trait(dfm, xcol=c_maf, ycol="mean_tmrca",
                     trait_col=c_trait, ax=ax,
                     title="MAF vs mean TMRCA (by trait)", regline=args.regline)
