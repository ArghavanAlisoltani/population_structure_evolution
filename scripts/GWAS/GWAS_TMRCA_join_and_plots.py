...
python GWAS_TMRCA_join_and_plots.py \
...
def merge_asof_per_chrom(gwas, tmrca, scaf_col, pos_col,
    ...
    mask rows where position > end_tmrca.
    Assumes segments for a scaffold are non-overlapping and sorted.
    """
    out = []
    for scaf, gsub in gwas.groupby(scaf_col, sort=False):
        tsub = tmrca[tmrca[t_scaf_col] == scaf]
        if tsub.empty:
            # keep rows with NaNs in TMRCA fields
            gsub = gsub.copy()
            out.append(gsub)
            continue
        gsub = gsub.sort_values(pos_col)
        tsub = tsub.sort_values(t_start_col)
        merged = pd.merge_asof(
            gsub, tsub,
            left_on=pos_col, right_on=t_start_col,
            direction="backward", allow_exact_matches=True
        )
        # keep only those truly inside the interval
        inside = merged[pos_col].between(merged[t_start_col], merged[t_end_col], inclusive="both")
        merged.loc[~inside, ["mean_tmrca","Low_CI","UP_CI","seg_length",
                             t_start_col, t_end_col]] = np.nan
        out.append(merged)
    return pd.concat(out, ignore_index=True) if out else gwas.copy()

def q4(series):
    # Q4 ~ 100th percentile, equivalent to max with NaN-safe handling
    return np.nanpercentile(series, 100.0)

def trait_summary(df, trait_col, tmrca_col):
    summ = (df
            .groupby(trait_col, dropna=False, observed=True)
            .apply(lambda g: pd.Series({
                "n_total": len(g),
                "n_with_tmrca": g[tmrca_col].notna().sum(),
                "median": np.nanmedian(g[tmrca_col]),
                "q1": np.nanpercentile(g[tmrca_col], 25) if g[tmrca_col].notna().any() else np.nan,
                "q3": np.nanpercentile(g[tmrca_col], 75) if g[tmrca_col].notna().any() else np.nan,
                "q4": q4(g[tmrca_col]) if g[tmrca_col].notna().any() else np.nan,
                "min": np.nanmin(g[tmrca_col]) if g[tmrca_col].notna().any() else np.nan,
                "max": np.nanmax(g[tmrca_col]) if g[tmrca_col].notna().any() else np.nan,
            }))
            .reset_index())
    return summ

def add_reg_line(ax, x, y, color="black", lw=1.2, alpha=0.8):
    # simple OLS line (ignores NaNs)
...
def scatter_basic(x, y, ax, title, xlabel, ylabel,
                  color="black", alpha=0.6, s=10, regline=False):
...
def scatter_by_trait(df, xcol, ycol, trait_col, ax, title, regline=False):
    traits = df[trait_col].astype(str).fillna("NA").unique().tolist()
    # pick a repeating palette
    cmap = cm.get_cmap("tab20", max(10, len(traits)))
    for i, t in enumerate(traits):
        sub = df[df[trait_col].astype(str).fillna("NA") == t]
        ax.scatter(sub[xcol], sub[ycol],
                   s=10, alpha=0.7, color=cmap(i), label=t, edgecolors="none")
        if regline:
            add_reg_line(ax, sub[xcol].to_numpy(), sub[ycol].to_numpy(), color=cmap(i))
    ax.set_title(title)
    ax.set_xlabel(xcol)
    ax.set_ylabel(ycol)
...
def main():
    ap = argparse.ArgumentParser(description="Annotate GWAS with TMRCA & summarize/plot")
    ap.add_argument("--gwas", required=True,
                    help="GWAS MTAG file (TSV) with columns: Significant_Trait, Scaffold, position, MAF, BETA, ...")
    ap.add_argument("--tmrca", required=True,
                    help="TMRCA TSV, columns: CHROM, start_tmrca, end_tmrca, mean_tmrca, Low_CI, UP_CI, seg_length")
    ap.add_argument("--outdir", default="gwas_tmrca_out")
    # column names (defaults match what you showed)
    ...
    # plotting options
    ap.add_argument("--regline", action="store_true", help="Overlay regression line(s)")
    ...
    gwas[c_pos] = pd.to_numeric(gwas[c_pos], errors="coerce")
    gwas[c_beta] = pd.to_numeric(gwas[c_beta], errors="coerce")
    gwas[c_maf] = pd.to_numeric(gwas[c_maf], errors="coerce")
    ...
    tmrca[t_lo]    = pd.to_numeric(tmrca[t_lo], errors="coerce")
    tmrca[t_hi]    = pd.to_numeric(tmrca[t_hi], errors="coerce")
    if t_len is not None:
        tmrca[t_len] = pd.to_numeric(tmrca[t_len], errors="coerce")

    gwas = gwas.dropna(subset=[c_scaf, c_pos])

    # ---- interval join (position ∈ [start_tmrca, end_tmrca])
    annotated = merge_asof_per_chrom(
        gwas, tmrca[[t_scaf, t_start, t_end, t_mean, t_lo, t_hi, *( [t_len] if t_len else [] )]].copy(),
        scaf_col=c_scaf, pos_col=c_pos,
        t_scaf_col=t_scaf, t_start_col=t_start, t_end_col=t_end
    )

    # keep a tidy column order
    tmrca_cols = [t_scaf, t_start, t_end, t_mean, t_lo, t_hi] + ([t_len] if t_len else [])
    # rename TMRCA columns to intuitive names in output
    rename_map = {
        t_scaf: "tmrca_scaffold",
        t_start: "start_tmrca",
        t_end: "end_tmrca",
        t_mean: "mean_tmrca",
        t_lo: "Low_CI",
    ...
    # ---- per-trait summary (only rows with mean_tmrca present)
    ...
    # MAF vs mean_tmrca (all, black)
    dfm = annotated.dropna(subset=["mean_tmrca", c_maf])
    fig, ax = plt.subplots(figsize=(7,5))
    scatter_basic(dfm[c_maf], dfm["mean_tmrca"], ax,
                  title="MAF vs mean TMRCA", xlabel="MAF", ylabel="mean TMRCA",
                  color="black", regline=args.regline)
    savefig(fig, "MAF_vs_TMRCA_all")

    # MAF vs mean_tmrca colored by trait
    fig, ax = plt.subplots(figsize=(8,5.5))
    scatter_by_trait(dfm, xcol=c_maf, ycol="mean_tmrca",
                     trait_col=c_trait, ax=ax,
                     title="MAF vs mean TMRCA (colored by trait)",
                     regline=args.regline)
    savefig(fig, "MAF_vs_TMRCA_byTrait")

    print(f"[OK] Wrote:\n  - {out_annot}\n  - {out_summ}\n  - figures to {args.outdir}/")
