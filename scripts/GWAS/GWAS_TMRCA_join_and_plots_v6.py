...
python GWAS_TMRCA_join_and_plots_v6.py \
  ...
  --outdir "gwas_MTAG_trait_tmrca_results_v6" \
...
python GWAS_TMRCA_join_and_plots_v6.py \
  ...
  --tmrca "/Users/aria/Desktop/OSU_projects/conifers/LP/ARGweaver/NOV_27_2025/all_tmrca_as_scaffold1.tsv" \
  --outdir "gwas_single_trait_tmrca_results_v6" \
  --allpoints-color "#000000" \
 --regline --show-lm --show-r --show-p \
  --stats-outside --stats-panel --stats-position top --stats-panel-height 1.6 \
  --min-n-trait 5 \
...
def make_axes(figsize=(7,5), stats_strip=False, strip_pos="top", strip_height_in=1.0):
    ...
    does NOT shrink the plotting area. The strip size is in inches.
    """
    if not stats_strip:
...
def summarize_snps_in_top(annotated, trait_col, mean_col, cutoffs):
    ...
    for P, cutoff in cutoffs.items():
        ...
        # also append boolean column to annotated
...
def main():
    ap = argparse.ArgumentParser(
        description="Annotate GWAS with TMRCA & summarize/plot (v6: top-percentile coverage using ALL TMRCA)."
    )
    ...
    # Figure sizing
    ap.add_argument("--fig-w", type=float, default=7.0, help="Base plot width (inches)")
    ap.add_argument("--fig-h", type=float, default=5.0, help="Base plot height (inches)")
    ...
    savefig(fig, "BETA_vs_TMRCA_all", dpi=args.dpi)
    ...
    scatter_basic(dfp["absBETA"].to_numpy(), dfp["mean_tmrca"].to_numpy(), ax,
                  ...
                  stats_margin=args.stats_margin,
                  stats_panel=args.stats_panel, stats_panel_ax=ax_strip)
    savefig(fig, "absBETA_vs_TMRCA_all", dpi=args.dpi)
    ...
    scatter_basic(dfm[c_maf].to_numpy(), dfm["mean_tmrca"].to_numpy(), ax,
                  ...
                  stats_margin=args.stats_margin,
                  stats_panel=args.stats_panel, stats_panel_ax=ax_strip)
    savefig(fig, "MAF_vs_TMRCA_all", dpi=args.dpi)

    # BY-TRAIT (with optional min-N filter)
    def filter_by_min_points(df, trait_col, xcol, ycol, min_n):
        if min_n <= 0 or df.empty:
            return df, pd.Series(dtype=int)
        dvalid = df[[trait_col, xcol, ycol]].dropna()
    ...
    scatter_by_trait(dfc_bt, xcol=c_beta, ycol="mean_tmrca",
                     ...
                     stats_margin=args.stats_margin,
                     stats_panel=args.stats_panel, stats_panel_ax=ax_strip)
    savefig(fig, "BETA_vs_TMRCA_byTrait", dpi=args.dpi)
    ...
    scatter_by_trait(dfc_abs_f, xcol="absBETA", ycol="mean_tmrca",
                     ...
                     stats_margin=args.stats_margin,
                     stats_panel=args.stats_panel, stats_panel_ax=ax_strip)
    savefig(fig, "absBETA_vs_TMRCA_byTrait", dpi=args.dpi)
    ...
    scatter_by_trait(dfc_maf_f, xcol=c_maf, ycol="mean_tmrca",
                     ...
                     stats_margin=args.stats_margin,
                     stats_panel=args.stats_panel, stats_panel_ax=ax_strip)
    savefig(fig, "MAF_vs_TMRCA_byTrait", dpi=args.dpi)
