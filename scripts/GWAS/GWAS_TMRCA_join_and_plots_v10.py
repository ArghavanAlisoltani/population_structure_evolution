...
python GWAS_TMRCA_join_and_plots_v12.py \
  ...
  --outdir "gwas_MTAG_trait_tmrca_results_v12" \
...
def scatter_basic(x, y, ax, title, xlabel, ylabel,
    ...
    # All points always plotted (including TMRCA zeros)
    ax.scatter(x, y, s=s, c=color, alpha=alpha, edgecolors="none")

    # Regression / stats use optional TMRCA reg filter
...
def scatter_by_trait(df, xcol, ycol, trait_col, ax, title,
                     ...
                     stats_traits=None,    # list of trait names to annotate on-plot
    ...
    """
    Colored-by-trait scatter. xcol/ycol are data columns, but x-axis label can be
    overridden by xlabel_override (e.g., 'TMRCA').
    stats_trait_xq / stats_trait_yq are quantiles (0-100) for per-trait stats text position.
    """
    ...
    # ----- overall stats (across all traits) using reg_filter
    ...
    for i, t in enumerate(traits):
        ...
        # choose alpha (highlight vs default)
        if str(t) in highlight_traits:
            alpha = alpha_highlight
        else:
            alpha = alpha_default

        # plot all points
        ax.scatter(x, y, s=10, alpha=alpha, color=col, label=str(t), edgecolors="none")

        # regression / stats for this trait (using optional reg_filter)
        mask = np.isfinite(x) & np.isfinite(y)
        if reg_filter is not None:
            mask &= (x > reg_filter)
        x_reg = x[mask]
        ...
        # per-trait stats in legend (if requested)
        ...
        # per-trait stats text on the plot for selected stats_traits
...
def make_all_plots(annotated, c_trait, c_beta, c_maf, args,
                   ...
                   exclude_traits=None):
    """
    Generate the full set of plots for a given annotated DF.
    """
    ...
    if not dfp.empty:
        ...
        scatter_basic(
            dfp["mean_tmrca"].to_numpy(), dfp[c_beta].to_numpy(), ax,
            title=f"TMRCA vs BETA{label_suffix}",
            xlabel="TMRCA", ylabel="BETA",
    ...
    if not dfp.empty:
        ...
        fig, ax, ax_strip = make_axes(base_size, stats_strip=args.stats_panel,
                                      ...
                                      strip_height_in=args.stats_panel_height)
        scatter_basic(
            dfp["mean_tmrca"].to_numpy(), dfp["absBETA"].to_numpy(), ax,
            title=f"TMRCA vs |BETA|{label_suffix}",
            xlabel="TMRCA", ylabel="|BETA|",
            color=args.allpoints_color, alpha=0.6, regline=args.regline,
            show_lm=args.show_lm, show_r=args.show_r, show_p=args.show_p,
    ...
    if not dfm.empty:
        fig, ax, ax_strip = make_axes(base_size, stats_strip=args.stats_panel,
                                      ...
                                      strip_height_in=args.stats_panel_height)
        scatter_basic(
            dfm["mean_tmrca"].to_numpy(), dfm[c_maf].to_numpy(), ax,
            title=f"TMRCA vs MAF{label_suffix}",
            xlabel="TMRCA", ylabel="MAF",
            color=args.allpoints_color, alpha=0.6, regline=args.regline,
            show_lm=args.show_lm, show_r=args.show_r, show_p=args.show_p,
            reg_filter=reg_filter,
            stats_outside=args.stats_outside, stats_position=args.stats_position,
    ...
    # Traits to exclude from colored plots
    ...
    if not dfc.empty:
        ...
        # Exclude selected traits AFTER min_n filter
        if exclude_traits:
        ...
        if not dfc_bt.empty:
            fig, ax, ax_strip = make_axes(base_size, stats_strip=args.stats_panel,
                                          ...
                                          strip_height_in=args.stats_panel_height)
            scatter_by_trait(
                dfc_bt, xcol="mean_tmrca", ycol=c_beta, trait_col=c_trait, ax=ax,
                title=f"TMRCA vs BETA (by trait; min_n={args.min_n_trait}){label_suffix}",
    ...
    if not dfc.empty:
        ...
        if not dfc_abs_f.empty:
            fig, ax, ax_strip = make_axes(base_size, stats_strip=args.stats_panel,
                                          ...
                                          strip_height_in=args.stats_panel_height)
            scatter_by_trait(
                dfc_abs_f, xcol="mean_tmrca", ycol="absBETA", trait_col=c_trait, ax=ax,
                title=f"TMRCA vs |BETA| (by trait; min_n={args.min_n_trait}){label_suffix}",
                regline=args.regline, show_lm=args.show_lm, show_r=args.show_r, show_p=args.show_p,
                reg_filter=reg_filter,
                per_trait_stats=args.per_trait_stats,
    ...
    if not dfc_maf.empty:
        ...
        if not dfc_maf_f.empty:
            fig, ax, ax_strip = make_axes(base_size, stats_strip=args.stats_panel,
                                          ...
                                          strip_height_in=args.stats_panel_height)
            scatter_by_trait(
                dfc_maf_f, xcol="mean_tmrca", ycol=c_maf, trait_col=c_trait, ax=ax,
                title=f"TMRCA vs MAF (by trait; min_n={args.min_n_trait}){label_suffix}",
                regline=args.regline, show_lm=args.show_lm, show_r=args.show_r, show_p=args.show_p,
                reg_filter=reg_filter,
                per_trait_stats=args.per_trait_stats,
                stats_traits=stats_traits,
...
def main():
    ap = argparse.ArgumentParser(
        description="Annotate GWAS with TMRCA & summarize/plot (v10: trait-specific alpha, colors, regfilter, stats position, exclude traits)."
    )
    ...
    ap.add_argument("--tmrca-filter", type=float, default=None,
                    help="If set, also generate a second set of plots using only rows with TMRCA > cutoff.")
    ...
    ap.add_argument("--tmrca-regfilter", type=float, default=None,
                    help="If set, regression lines/stats use only points with TMRCA > this threshold, "
                         "while all points are still plotted.")
    ...
    # NEW: allow excluding traits from colored-by-trait figures entirely
    ...
    # NEW: control per-trait stats text position (quantiles of x/y within trait)
    ...
    c_trait = pick_col(gcols, [args.trait-col if False else args.trait_col], label="trait")  # trick to avoid hyphen confusion
    ...
    # ===================== percent of SNPs in top-X% (full data) =====================
    try:
    ...
    # ===================== plots: full data =====================
    make_all_plots(
        annotated, c_trait, c_beta, c_maf, args,
        label_suffix="", fname_suffix="",
    ...
    # ===================== plots: filtered by TMRCA cutoff (optional) =====================
    if args.tmrca_filter is not None:
        ...
        if not ann_filt.empty:
            ...
            make_all_plots(
                ann_filt, c_trait, c_beta, c_maf, args,
