...
def main():
    ap = argparse.ArgumentParser(
        description=(
            ...
            "(v14: adds option to drop duplicated SNPs before analysis)."
        )
    )
    ...
    ap.add_argument("--snp-col",   default="SNP",
                    help="Name of the SNP ID column (for deduplicating SNPs).")

    # Option to drop duplicate SNP IDs before downstream analysis
    ap.add_argument("--drop-dup-snps", action="store_true",
                    help="If set, drop duplicated SNPs (by SNP column) keeping the first occurrence.")
    ...
    c_snp   = pick_col(gcols, [args.snp_col], required=False, label="SNP")
    ...
    # -------- optional: drop duplicate SNP IDs before anything else
    if args.drop_dup_snps:
        if c_snp is None:
            raise SystemExit("ERROR: --drop-dup-snps was set, but SNP column was not found.")
        n_before = len(gwas)
        # keep first occurrence, preserving original ordering as much as possible
        gwas = gwas.drop_duplicates(subset=[c_snp], keep="first")
        n_after = len(gwas)
        print(f"[INFO] Dropped {n_before - n_after} duplicated SNP rows based on column '{c_snp}'.")

    # drop rows missing scaffold/position for the merge
