...
scaffold <- "scaffold_1a"        # <- put your scaffold/contig name here (must match CHROM)
start_bp <- 1          # <- region start (bp)
end_bp   <- 1500000000         # <- region end (bp)
...
y_pad <- y_max * 0.9   # extra headroom to draw the gene track
...
ggsave(paste0("1a_Per-site FST on ", scaffold, " (",scales::comma(start_bp), "–", scales::comma(end_bp), ").png"),
