...
def tests_for_comp(dfw: pd.DataFrame, lab: str) -> pd.DataFrame:
    ...
    # rho ≈ (phi - 1) / (n_bar - 1), where phi ≈ Pearson dispersion of Binomial
    ...
    # Var(K) ≈ μ + μ^2/θ  ⇒  θ ≈ sum(μ^2) / sum((K-μ)^2 - μ), clipped
    ...
    if np.isfinite(theta_est) and np.any(np.isfinite(mu)):
        ok = mu >= 0
        # scipy nbinom params: n=r, p=r/(r+mu)
        r = theta_est
    ...
    out = pd.DataFrame({
        ...
        f"p_bin_{lab}": p_bin,
        f"q_bin_{lab}": bh_fdr(p_bin),
        f"p_poi_{lab}": p_poi,
        f"q_poi_{lab}": bh_fdr(p_poi),
        f"p_bb_{lab}":  p_bb,
        f"q_bb_{lab}":  bh_fdr(p_bb),
        f"p_nb_{lab}":  p_nb,
        f"q_nb_{lab}":  bh_fdr(p_nb),
    })
    return out
...
def make_manhattan(dfw: pd.DataFrame, qcol: str, title: str, out_png: Path, top_n: int = 10):
    if qcol not in dfw.columns or dfw.empty:
        return None
    # Order scaffolds numerically
    ...
    # gray for non-sig, black for sig
    plt.scatter(dd.loc[~dd["sig"], "cumpos"], dd.loc[~dd["sig"], "ml10"],
                s=6, c="#bdbdbd", linewidths=0)
    plt.scatter(dd.loc[dd["sig"], "cumpos"], dd.loc[dd["sig"], "ml10"],
                s=8, c="black", linewidths=0)
    # top10 in red with labels
    plt.scatter(dd_top["cumpos"], dd_top["ml10"], s=22, c="red", linewidths=0, zorder=5)
    for _, r in dd_top.iterrows():
        plt.text(r["cumpos"], r["ml10"] + 0.1, f"{r['CHROM']}:{int(r['WIN_START'])}-{int(r['WIN_END'])}",
                 fontsize=7, color="red", rotation=45, ha="left", va="bottom")

    plt.title(title)
    plt.ylabel(r"$-\log_{10}(q)$")
    plt.xlabel("Genome (ordered scaffolds)")
...
def add_slide(prs, title, bullets=None, image=None):
    ...
    if bullets:
        tb = slide.shapes.add_textbox(Inches(0.5), Inches(1.3), Inches(9), Inches(3.5))
        tf = tb.text_frame
        tf.clear()
...
def make_report(all_summaries: list, pptx_path: Path, comparison_imgs: list[Path]):
    ...
    slide.placeholders[1].text = (
        ...
        "Includes dispersion diagnostics and Manhattan plots"
    )
    # Per-combo slides
    for s in all_summaries:
        ...
        bullets = [
            ...
            f"Output: {s['csv_name']}",
            "Manhattan plots (POI vs NB) saved in combo folder."
        ]
        add_slide(prs, title, bullets=bullets, image=s.get("plot_path"))
    # Comparison slides
...
def main():
    ...
    for win_size, overlap in itertools.product(WIN_SIZES, OVERLAPS):
        ...
        def comp_nums(prefix: str):
            ...
            # How many windows have finite q-values?
            n_qs_bin = int(np.isfinite(dfw[f"q_bin_{prefix}"]).sum())
            n_qs_poi = int(np.isfinite(dfw[f"q_poi_{prefix}"]).sum())
            n_qs_bb  = int(np.isfinite(dfw[f"q_bb_{prefix}"]).sum())
            n_qs_nb  = int(np.isfinite(dfw[f"q_nb_{prefix}"]).sum())
            return d_bin, d_poi, rho, theta, ns_bin, ns_poi, ns_bb, ns_nb, n_qs_bin, n_qs_poi, n_qs_bb, n_qs_nb
        ...
        plot_path = save_ratio_plot(dfw, SCAFFOLD_OF_INTEREST, win_size, overlap, combo_dir)

        # Manhattan plots: POI vs NB for each comparison
        man_imgs = []
        for comp, label in [("3vs4","3vs4"), ("3vs5","3vs5"), ("4vs5","4vs5")]:
            # Ensure WIN_MID exists
            if "WIN_MID" not in dfw.columns:
                dfw["WIN_MID"] = ((dfw["WIN_START"] + dfw["WIN_END"]) // 2).astype(np.int64)
            poi_png = combo_dir / f"manhattan_POI_{label}_win{win_size}_ov{overlap}.png"
            nb_png  = combo_dir / f"manhattan_NB_{label}_win{win_size}_ov{overlap}.png"
            make_manhattan(dfw, f"q_poi_{comp}", f"Poisson FDR — {label}\nwin={win_size:,}, ov={overlap:,}", poi_png)
            make_manhattan(dfw, f"q_nb_{comp}",  f"NegBin FDR — {label}\nwin={win_size:,}, ov={overlap:,}",  nb_png)
            man_imgs.extend([poi_png, nb_png])
        ...
        summary = {
            ...
            # number of windows with finite q-values (comparison you asked)
            "n_qvals_bin_3vs4": c34[8], "n_qvals_poi_3vs4": c34[9], "n_qvals_bb_3vs4": c34[10], "n_qvals_nb_3vs4": c34[11],
            "n_qvals_bin_3vs5": c35[8], "n_qvals_poi_3vs5": c35[9], "n_qvals_bb_3vs5": c35[10], "n_qvals_nb_3vs5": c35[11],
            "n_qvals_bin_4vs5": c45[8], "n_qvals_poi_4vs5": c45[9], "n_qvals_bb_4vs5": c45[10], "n_qvals_nb_4vs5": c45[11],
        ...
        with open(combo_dir / "summary.txt", "w") as fh:
            ...
            fh.write("Finite q-values count BIN/POI/BB/NB for each comparison included in overall summary.\n")
