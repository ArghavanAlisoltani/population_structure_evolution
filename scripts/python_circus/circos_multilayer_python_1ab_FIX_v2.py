# circos_multilayer_python_1ab_FIX_v2.py
# Multi-layer Circos with robust I/O, scaffold_1a/1b merge-split, LG mapping,
# and SAFE handling of meta columns for both intervals and points.
...
SNP_WINDOW_BP    = 1_000_000  # 1 Mb

SHOW_TE          = True
TE_MAJOR_TO_SHOW = {"LTR", "DNA", "Helitron", "LINE", "SINE"}   # majors only
TE_COLORS = {
    "LTR:Gypsy":  "#1b9e77",
    "LTR:Copia":  "#d95f02",
    "DNA":        "#7570b3",
...
def normalize_scaffold_val(x: str) -> str:
    """Return canonical scaffold like 'scaffold_1' or 'scaffold_1a'."""
    if pd.isna(x):
        return np.nan
    s = str(x).strip()
    s_low = s.lower()
...
# ===============================
# Robust loaders (standardize columns)
# ===============================
...
# ===============================
# 1a/1b merge/split helpers (intervals AND points with meta)
# ===============================
def merge_1ab_points_df(df: pd.DataFrame) -> pd.DataFrame:
    """Unify 1a/1b into scaffold_1 (1b positions shifted by OFFSET-1)."""
    out = df.copy()
    orig_chrom = out["CHROM"].astype(str)
    out.loc[orig_chrom == CHR1B, "POS"] = out.loc[orig_chrom == CHR1B, "POS"].astype(float) + (OFFSET - 1.0)
    out.loc[orig_chrom.isin([CHR1A, CHR1B, CHR1]), "CHROM"] = CHR1
    return out

def split_1_to_1ab_points_with_meta(df: pd.DataFrame, meta_cols: list[str] | None = None) -> pd.DataFrame:
    """Split scaffold_1 points into 1a/1b, replicating meta columns."""
    if meta_cols is None: meta_cols = []
    df = df.copy()
    rows = []

    mask_a = (df["CHROM"] == CHR1) & (df["POS"].astype(float) <= GAP_END)
    if mask_a.any():
        a = df.loc[mask_a, ["CHROM","POS"] + meta_cols].copy()
        a["CHROM"] = CHR1A
        rows.append(a)

    mask_b = (df["CHROM"] == CHR1) & (df["POS"].astype(float) >= OFFSET)
    if mask_b.any():
        b = df.loc[mask_b, ["CHROM","POS"] + meta_cols].copy()
        b["CHROM"] = CHR1B
...
def map_points_to_LG_with_meta(df_points_std: pd.DataFrame, scaff_info: pd.DataFrame, meta_cols: list[str] | None = None) -> pd.DataFrame:
    """Map points (CHROM, POS, + meta) to LG coords; replicates meta if splitting 1→1a/1b."""
    if meta_cols is None: meta_cols = []
...
# 5) Trait GWAS (FDR, MAF filter) — CARRY FDR META through split (fix)
...
if SHOW_SNP_DENSITY and not snp_win.empty:
    ...
    for lg in lg_order:
        ...
        tr = circos.sectors[lg].add_track((80, 90))
        tr.axis()
...
present_te_labels = [k for k in ["LTR:Gypsy","LTR:Copia","DNA","Helitron","LINE","SINE"] if k in present_te_labels]
