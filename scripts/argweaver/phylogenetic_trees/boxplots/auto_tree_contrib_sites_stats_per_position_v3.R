...
pick_window <- function(smc_dir, scaffold_query, position_query, recursive=TRUE){
  ...
  dt <- dt[scaffold == scaffold_query & start <= position_query & position_query <= end]
  if (nrow(dt)==0){
    stop("No window found for scaffold=", scaffold_query,
         " containing position=", position_query,
         " in: ", smc_dir)
  }
...
read_smc_trees <- function(smc_file){
  ...
  if (any(vapply(sp, length, integer(1)) < 4)){
    sp <- strsplit(keep, "\\s+")
  }
...
# -----------------------------
allele_overall_by_position <- function(geno){
  a <- rbind(
    geno[!is.na(allele1), .(position, allele=allele1)],
    geno[!is.na(allele2), .(position, allele=allele2)]
  )
  a[, .(Count=.N), by=.(position, allele)][order(position, -Count)]
}

genotype_overall_by_position <- function(geno){
  geno[!is.na(genotype), .(Count=.N), by=.(position, genotype)][order(position, -Count)]
}

het_hom_overall_by_position <- function(geno){
  geno[!is.na(is_het),
       .(Count=.N),
       by=.(position, state=ifelse(is_het,"HET","HOM"))][order(position, state)]
}

allele_by_group_by_position <- function(geno, meta, group_col){
  dt <- merge(geno, meta, by="indiv", all.x=TRUE)
  a <- rbind(
    dt[!is.na(allele1), .(position, group=get(group_col), allele=allele1)],
    dt[!is.na(allele2), .(position, group=get(group_col), allele=allele2)]
  )
  a[, .(Count=.N), by=.(position, group, allele)][order(position, group, -Count)]
}

genotype_by_group_by_position <- function(geno, meta, group_col){
  dt <- merge(geno, meta, by="indiv", all.x=TRUE)
  dt <- dt[!is.na(genotype)]
  dt[, .(Count=.N), by=.(position, group=get(group_col), genotype)][order(position, group, -Count)]
}
...
trait      <- get_arg(args, "--trait", "C13")  # not required for counts; kept for metadata outputs if desired
rep_str    <- get_arg(args, "--replicates", "0")
outbase    <- get_arg(args, "--outbase", "tree_contrib_stats_per_position")
...
for (rep in reps) {
  ...
  # positions contributing to selected tree interval
  ...
  diag <- data.table(
    ...
    n_sites_in_selected_tree=length(contrib_pos)
  )
  fwrite(diag, file.path(rep_out, "rep_summary_diagnostics.tsv"), sep="\t")

  if (length(contrib_idx) == 0) {
    cat("  NOTE: 0 sites mapped to selected tree interval. See diagnostics in: ", rep_out, "\n", sep="")
    next
  }

  # Build genotype table for selected positions only
  hap_long <- sites_to_hap_long(hap_names, contrib_pos, allele_str[contrib_idx])
  geno <- hap_long_to_genotypes(hap_long)

  # Optional: full long table with meta columns
  if (write_long) {
  ...
  # ---- PER-POSITION overall summaries
  fwrite(allele_overall_by_position(geno),
         file.path(rep_out, "allele_overall_by_position.tsv"), sep="\t")
  fwrite(genotype_overall_by_position(geno),
         file.path(rep_out, "genotype_overall_by_position.tsv"), sep="\t")
  fwrite(het_hom_overall_by_position(geno),
         file.path(rep_out, "het_hom_overall_by_position.tsv"), sep="\t")

  # ---- PER-POSITION by group summaries
  if (proc_col %in% names(meta)) {
    fwrite(allele_by_group_by_position(geno, meta, proc_col),
           file.path(rep_out, "allele_by_provenance_by_position.tsv"), sep="\t")
    fwrite(genotype_by_group_by_position(geno, meta, proc_col),
           file.path(rep_out, "genotype_by_provenance_by_position.tsv"), sep="\t")
  }
  if (site_col %in% names(meta)) {
    fwrite(allele_by_group_by_position(geno, meta, site_col),
           file.path(rep_out, "allele_by_site_by_position.tsv"), sep="\t")
    fwrite(genotype_by_group_by_position(geno, meta, site_col),
           file.path(rep_out, "genotype_by_site_by_position.tsv"), sep="\t")
  }
  if (mum_col %in% names(meta)) {
    fwrite(allele_by_group_by_position(geno, meta, mum_col),
           file.path(rep_out, "allele_by_mum_by_position.tsv"), sep="\t")
    fwrite(genotype_by_group_by_position(geno, meta, mum_col),
           file.path(rep_out, "genotype_by_mum_by_position.tsv"), sep="\t")
  }

  cat("  Wrote per-position summaries to: ", rep_out, "\n", sep="")
}
