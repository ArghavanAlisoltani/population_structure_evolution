...
#   • TWO Manhattan plots: linear Y and log10 Y
#   • NO gray scaffold bands; points < THRESH_FADE get transparent alpha
#   • Top-K windows highlighted in red
#   • PowerPoint deck with figures and tables
# ------------------------------------------------------------
...
def plot_manhattan(win_all, scaffolds_ordered, chrom_ticks, yscale, out_png, title_suffix):
    """Scatter without gray bands; fade points below threshold; highlight top-K in red."""
    # Determine alpha per point (fade if below threshold)
...
# Per-scaffold stats table (optional)
s2 = prs.slides.add_slide(prs.slide_layouts[5])
s2.shapes.title.text = "Per-scaffold summary (generations)"
per_csv = os.path.join(OUTDIR, "summary_by_scaffold.csv")
per_scaf.to_csv(per_csv, index=False)
# embed first ~20 rows for readability
sub = per_scaf.head(20).copy()
...
s5.shapes.add_picture(hist_path, Inches(0.5), Inches(1.2), width=Inches(9))

pptx_path = os.path.join(OUTDIR, "tmrca_summary_slides_v2.pptx")
prs.save(pptx_path)
