...
def summarize_te_streaming(te_path: str, te_chrom: str, te_class_col: str, te_start: str, te_end: str,
    ...
    # hierarchy mapping
    map_rows = [{"sequence_ontology": term,
                 "Class": infer_te_hierarchy(term)[0],
                 "Order": infer_te_hierarchy(term)[1],
                 "Superfamily": infer_te_hierarchy(term)[2]} for term in sorted(unique_terms)]
    hier_df = pd.DataFrame(map_rows, columns=["sequence_ontology","Class","Order","Superfamily"])
    ...
    for scf in scaffolds:
        recs.append({
            "scaffold": scf,
            "n_TEs_total": total_counts_per_scf.get(scf, 0),
            "n_TE_classes": len(class_set_per_scf.get(scf, set())),
            "n_TE_superfamilies": len(sf_set_per_scf.get(scf, set())),
            "te_coverage_bp": int(te_union_bp.get(scf, 0)),
        })
    # Guarantee columns even if empty
    te_summary_df = pd.DataFrame(recs, columns=["scaffold","n_TEs_total","n_TE_classes","n_TE_superfamilies","te_coverage_bp"])
    ...
    if scaffolds:
        ...
        te_class_long = pd.DataFrame(rows, columns=["scaffold","TE_class","count"])
        if not te_class_long.empty:
            te_class_long["TEclass_col"] = te_class_long["TE_class"].map(lambda x: "TEclass_"+sanitize(x))
            te_class_wide = (te_class_long.pivot_table(index="scaffold", columns="TEclass_col",
        ...
        te_sf_long = pd.DataFrame(rows, columns=["scaffold","Superfamily","count"])
        if not te_sf_long.empty:
            te_sf_long["TEsf_col"] = te_sf_long["Superfamily"].map(lambda x: "TEsf_"+sanitize(x))
            te_sf_wide = (te_sf_long.pivot_table(index="scaffold", columns="TEsf_col",
                                                 values="count", fill_value=0, aggfunc="sum")
                                          .reset_index())
        else:
...
def main():
    ap = argparse.ArgumentParser(
        description=("V5a: Fast per-scaffold summaries for TMRCA, genes, and massive TE tables by streaming; "
                     "then merged. Robust to empty TE summaries; writes all outputs to a folder.")
    )
    ...
    # Ensure these have a scaffold column even if empty
    if "scaffold" not in te_summary_df.columns:
        te_summary_df["scaffold"] = pd.Series(dtype=str)
    if "scaffold" not in te_class_wide.columns:
        te_class_wide["scaffold"] = pd.Series(dtype=str)
    if "scaffold" not in te_sf_wide.columns:
        te_sf_wide["scaffold"] = pd.Series(dtype=str)
    ...
    scaff_all = set()
    for df in (tmrca_summary, mrna_summary, te_summary_df):
        if isinstance(df, pd.DataFrame) and ("scaffold" in df.columns):
            scaff_all |= set(df["scaffold"].dropna().astype(str).unique())
    base = pd.DataFrame({"scaffold": sorted(scaff_all)})
    ...
    if {"top_tmrca_coverage_bp","tmrca_coverage_bp"} <= set(out.columns):
        out["fraction_top_tmrca_bp"] = frac(out["top_tmrca_coverage_bp"], out["tmrca_coverage_bp"])
    if {"gene_coverage_bp","te_coverage_bp"} <= set(out.columns):
    ...
    if targets:
        ...
        # pooled TMRCA stats from raw table
    ...
    final_path = os.path.join(outdir, "scaffold_summary_v5a.tsv")
    out.to_csv(final_path, sep="\t", index=False)

    with open(os.path.join(outdir, "README_v5a.txt"), "w") as fh:
        fh.write("V5a per-scaffold summaries (robust to empty TE summaries)\n")
