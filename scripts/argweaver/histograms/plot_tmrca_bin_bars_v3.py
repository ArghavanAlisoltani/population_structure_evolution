...
v3:
- Same logic as v2 / original v1
- Optional legend position via --legend-loc
- Optional smoothing of area plots via --smooth-area-window
"""
...
def dual_axis_plot(
    ...
    # X positions / labels
    x = np.arange(len(df))
    labels = df[bin_col].astype(str).tolist()
    ...
    def _add_legend(fig, ax_main_for_legend, ax_sec_for_legend):
        ...
        if loc_str == "right-out":
            ...
            # Use matplotlib location strings directly
            fig.legend(
                handles_main + handles_sec,
                labels_main + labels_sec,
                loc=legend_loc,
...
def main():
    ...
    df = df.copy()
    df[args.col_bin] = pd.Categorical(df[args.col_bin],
