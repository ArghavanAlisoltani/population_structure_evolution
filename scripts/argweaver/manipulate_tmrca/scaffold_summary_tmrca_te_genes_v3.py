...
def main():
    ...
    # segment length (1-based inclusive)
    ...
    # TMRCA coverage (union bp) & median seg length
    ...
    # Fill count/coverage NaNs with 0; keep TMRCA stats as floats (NaN ok if no TMRCA on that scaffold)
    ...
    # derive fractions/ratios
    out["te_gene_coverage_ratio"] = np.where((out.get("gene_coverage_bp",0)+0)>0,
                                             out.get("te_coverage_bp",0) / out.get("gene_coverage_bp",1).replace(0,np.nan),
                                             np.nan)
    out["te_fraction_of_covered_bp"] = np.where(
        (out.get("gene_coverage_bp",0) + out.get("te_coverage_bp",0)) > 0,
        out.get("te_coverage_bp",0) / (out.get("gene_coverage_bp",0) + out.get("te_coverage_bp",0)),
        np.nan
    )
    out["fraction_top_tmrca_bp"] = np.where(out.get("tmrca_coverage_bp",0) > 0,
                                            out.get("top_tmrca_coverage_bp",0) / out.get("tmrca_coverage_bp",1),
                                            np.nan)
    # TMRCA variability
    # (cv = sd/mean, computed from per-scaffold series; where mean is NaN or 0, cv is NaN)
    tm_per_scf = tdf.groupby(args.tmrca_chrom)[args.tmrca_mean].apply(lambda s: pd.Series({
        "tmrca_sd": float(np.nanstd(s, ddof=1)) if s.notna().sum() >= 2 else np.nan
    })).reset_index().rename(columns={args.tmrca_chrom:"scaffold"})
    out = out.merge(tm_per_scf, on="scaffold", how="left")
    ...
    def add_combined_row(out_df: pd.DataFrame, tdf_tmrca: pd.DataFrame, gdf_mrna: pd.DataFrame, tte_te: pd.DataFrame):
        a, b, new = "scaffold_1a", "scaffold_1b", "scaffold_1"
        if (a not in out_df["scaffold"].values) and (b not in out_df["scaffold"].values):
            return out_df  # nothing to combine
        # gather TMRCA values across a+b
        tm_vals = tdf_tmrca.loc[tdf_tmrca[args.tmrca_chrom].isin([a,b]), args.tmrca_mean].dropna().to_numpy()
        n_tm = tdf_tmrca.loc[tdf_tmrca[args.tmrca_chrom].isin([a,b])].shape[0]
        # union coverages: since a and b are distinct scaffolds, sums are fine
        ...
        # Helper to sum safe
        def sget(r, col):
            return int(r[col]) if (r is not None and col in r.index and pd.notna(r[col])) else 0

        combined = dict(scaffold=new)
        # TMRCA counts/coverage
        combined["n_tmrca_segments"]     = sget(row_a,"n_tmrca_segments") + sget(row_b,"n_tmrca_segments")
        combined["tmrca_coverage_bp"]    = sget(row_a,"tmrca_coverage_bp") + sget(row_b,"tmrca_coverage_bp")
        combined["n_top_tmrca_segments"] = sget(row_a,"n_top_tmrca_segments") + sget(row_b,"n_top_tmrca_segments")
        combined["n_non_top_tmrca_segments"] = sget(row_a,"n_non_top_tmrca_segments") + sget(row_b,"n_non_top_tmrca_segments")
        combined["top_tmrca_coverage_bp"]= sget(row_a,"top_tmrca_coverage_bp") + sget(row_b,"top_tmrca_coverage_bp")
        # TMRCA stats from pooled values
        if tm_vals.size > 0:
            combined.update({
            ...
            combined.update({
                "tmrca_min": np.nan, "tmrca_q1": np.nan, "tmrca_median": np.nan,
                "tmrca_q3": np.nan, "tmrca_max": np.nan, "tmrca_mean": np.nan
            })
        # seg len stats (median/mean of pooled)
        seg_len_vals = (tdf_tmrca.loc[tdf_tmrca[args.tmrca_chrom].isin([a,b]), "seg_len"]).dropna().to_numpy()
        combined["median_seg_len"] = float(np.nanmedian(seg_len_vals)) if seg_len_vals.size>0 else np.nan
        combined["mean_seg_len"]   = float(np.nanmean(seg_len_vals))   if seg_len_vals.size>0 else np.nan
        # gene & TE coverage/counts
        combined["n_genes"]          = sget(row_a,"n_genes") + sget(row_b,"n_genes")
        combined["gene_coverage_bp"] = sget(row_a,"gene_coverage_bp") + sget(row_b,"gene_coverage_bp")
        combined["n_TEs_total"]      = sget(row_a,"n_TEs_total") + sget(row_b,"n_TEs_total")
        combined["n_TE_classes"]     = sget(row_a,"n_TE_classes") + sget(row_b,"n_TE_classes")  # note: this 'sum' is naive; distinct class IDs across scaff may overlap
        combined["te_coverage_bp"]   = sget(row_a,"te_coverage_bp") + sget(row_b,"te_coverage_bp")
        # derived
        ...
        # tmrca sd/cv
        if tm_vals.size >= 2:
            sd = float(np.nanstd(tm_vals, ddof=1))
        else:
            sd = np.nan
        combined["tmrca_sd"] = sd
        combined["tmrca_cv"] = (sd/combined["tmrca_mean"]) if (pd.notna(sd) and pd.notna(combined["tmrca_mean"]) and combined["tmrca_mean"]!=0) else np.nan
        # TE-class columns: sum the wide columns present
        tecols = [c for c in out_df.columns if c.startswith("TEclass_")]
        for c in tecols:
            combined[c] = sget(row_a,c) + sget(row_b,c)

        new_row = pd.DataFrame([combined])
        # place combined row at end
    ...
    out = add_combined_row(out, tdf, gdf, tte)
    ...
    # Also drop a tiny README of the top-percentile threshold used
    meta_path = re.sub(r"\.tsv$", ".meta.txt", out_path)
    with open(meta_path, "w") as fh:
        fh.write(f"Global top-percentile for high TMRCA: {args.top_percentile}%\n")
        fh.write(f"Threshold (mean_tmrca >=): {top_thr}\n")
        fh.write("Coverage metrics use union of intervals (1-based inclusive lengths).\n")
        fh.write("scaffold_1 row is a simple sum/pooled-statistics of scaffold_1a and scaffold_1b.\n")
