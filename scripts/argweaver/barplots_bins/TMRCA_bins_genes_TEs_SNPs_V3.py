#!/usr/bin/env python3
"""
TMRCA_bins_genes_TEs_SNPs_V3.py
...
def build_segments_by_scaffold(seg_df, scaf_col, start_col, end_col, mean_col):
    """
    Build a dict: scaffold -> (starts, ends, means, prefix_means) numpy arrays, sorted by start.
    """
    seg_dict = {}
    for scaf, sub in seg_df.groupby(scaf_col, sort=False):
        ...
        starts = sub[start_col].to_numpy(dtype="float64")
        ends   = sub[end_col].to_numpy(dtype="float64")
        means  = sub[mean_col].to_numpy(dtype="float64")
        prefix = np.cumsum(means)  # prefix sums for fast averaging

        seg_dict[str(scaf)] = (starts, ends, means, prefix)
    return seg_dict


def assign_interval_tmrca_vectorized(interval_df,
                                     iv_scaf, iv_start, iv_end,
                                     seg_dict,
                                     out_mean_col="tmrca_avg",
                                     out_nseg_col="n_segments_overlapped"):
    """
    Vectorized per-scaffold assignment of TMRCA to intervals (genes/TEs):

    For each scaffold:
      - For all intervals on that scaffold, compute left/right segment indices
        using np.searchsorted on ends/starts.
      - Use prefix sums of segment means to compute mean over [left:right]
        in O(1) per interval, no Python row loops.
    """
    df = interval_df.copy()
    df[out_mean_col] = np.nan
    df[out_nseg_col] = 0

    for scaf, sub in df.groupby(iv_scaf, sort=False):
        key = str(scaf)
        if key not in seg_dict:
            continue

        seg_starts, seg_ends, seg_means, seg_prefix = seg_dict[key]
        n_seg = len(seg_means)
        if n_seg == 0:
            continue

        idx = sub.index
        iv_start_arr = sub[iv_start].to_numpy(dtype="float64")
        iv_end_arr   = sub[iv_end].to_numpy(dtype="float64")

        # searchsorted over all intervals at once
        left  = np.searchsorted(seg_ends, iv_start_arr, side="left")
        right = np.searchsorted(seg_starts, iv_end_arr, side="right") - 1

        valid = (
            np.isfinite(iv_start_arr) &
            np.isfinite(iv_end_arr) &
            (left <= right) &
            (right >= 0) &
            (left < n_seg)
        )
        ...
        left_clip  = np.clip(left, 0, n_seg - 1)
        right_clip = np.clip(right, 0, n_seg - 1)

        sum_right_all = seg_prefix[right_clip]
        sum_left_all  = np.where(left_clip > 0, seg_prefix[left_clip - 1], 0.0)
        sums_all      = sum_right_all - sum_left_all

        nseg_all = (right - left + 1).astype("int64")

        avg = np.full_like(iv_start_arr, np.nan, dtype="float64")
        nseg = np.zeros_like(iv_start_arr, dtype="int64")

        avg[valid]  = sums_all[valid] / nseg_all[valid]
        nseg[valid] = nseg_all[valid]

        df.loc[idx, out_mean_col] = avg
        df.loc[idx, out_nseg_col] = nseg

    return df


def build_tmrca_bins(tmrca_values, bins_str=None, nbins=100):
    """
    Build TMRCA bin edges (linear scale, but usually log10 spaced).
...
def count_in_bins(values, bins_edges):
    """
    Count how many finite values fall into each bin (left-closed, right-open).
    """
...
def main():
    p = argparse.ArgumentParser(
        description=(
            "FAST V3: Compute average TMRCA per gene and TE, then bin TMRCA values and "
    ...
    p.add_argument("--nbins", type=int, default=100,
                   help="Number of automatic bins if --bins is not given (default: 100).")

    # output
    p.add_argument("--outdir", default="tmrca_bins_out_v3",
    ...
    mrna_tm = assign_interval_tmrca_vectorized(
        mrna,
    ...
    te_tm = assign_interval_tmrca_vectorized(
        te,
