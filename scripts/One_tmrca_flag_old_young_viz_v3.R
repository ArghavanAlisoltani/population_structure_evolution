...
# ---------- USER SETTINGS ----------
tmrca_path <- "All_tmrca_60_run.txt"     # path to one TMRCA file
  ann_xlsx   <- "~/Desktop/OSU_projects/conifers/LP/lodgepole_pine_assembly/Aria_curated_annotation_1ab_one_tissue.xlsx"                       # annotation workbook

contig   <- "scaffold_10"  # scaffold to visualize
x_start  <- 1     # start (bp) 7610823
x_end    <- 750000000      # end   (bp) 8300837

arg_start_is_zero_based <- TRUE
win <- 1000              # window size (bp)
...
winDT <- winDT %>% mutate(mid_bp = (w_start + w_end)/2, mid_mb = mid_bp/1e3)

# CI bounds for a shaded band (keep >0 for log scale)
winDT <- winDT %>%
  mutate(
    ci_low_line  = pmax(y_wmean - ciw_wmean/2, .Machine$double.eps),
    ci_high_line = y_wmean + ciw_wmean/2
  )


winDT <- winDT %>%
  mutate(mid_bp = (w_start + w_end)/2, mid_mb = mid_bp/1e3) %>%
  mutate(ci_low_line  = pmax(y_wmean - ciw_wmean/2, .Machine$double.eps),
         ci_high_line = y_wmean + ciw_wmean/2)
# ---------- FLAGS (young/mid/old) ----------
winDT <- winDT %>%
  group_by(chrom) %>%
...
g <- genes %>%
  ...
  mutate(
    xmin   = pmax(gene_start, x_start)/1e3,
    xmax   = pmin(gene_end,   x_end)/1e3,
    xmid   = (xmin + xmax)/2,
...
p <- ggplot(winDT, aes(x = mid_mb, y = y_wmean)) +
  geom_ribbon(aes(ymin = ci_low_line, ymax = ci_high_line),
              fill = "grey60", alpha = 0.35, color = NA) +
  ...
  labs(
    x = sprintf("%s position (Kb)", contig),
